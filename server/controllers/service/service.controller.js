...
const getServiceById = async (req, res) => {
  try {
    ...
    if (!service) {
      // If not found by ID, also check if this ID is a slug (fallback for robust frontend routing)
...
const createService = async (req, res) => {
  try {
    ...
    // Default providerId to authenticated user if not provided
    if (!providerId) {
      providerId = req.user.id;
    }

    // Check permissions: only admin or the user themselves can set their providerId
...
const updateService = async (req, res) => {
  try {
    ...
    // Check if service exists and user is owner or admin
    const service = await serviceService.getServiceById(id);
    if (!service) {
      return res.status(404).json({
        success: false,
