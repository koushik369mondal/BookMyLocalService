...
const mockNotifications = [
  ...
  { id: 3, title: "Payment Received", desc: "Receipt of $55.00 generated for Window Washing", time: "3 hours ago", icon: Sparkles, iconColor: "text-amber-400 bg-amber-500/10", unread: true },
...
export default function Navbar() {
  ...
  return (
    <div className="sticky top-0 z-50 w-full font-sans">
      ...
      {/* {showAnnouncementBar && (
        <div className="bg-gradient-to-r from-amber-500 via-orange-500 to-amber-600 text-slate-950 px-4 py-2 text-xs font-bold flex items-center justify-between shadow-xs transition-all duration-300 relative z-55">
          <div className="flex-1 flex items-center justify-center gap-2 text-center">
            <span className="bg-slate-950 text-amber-400 text-[10px] uppercase tracking-wider font-extrabold px-2 py-0.5 rounded-md shrink-0">
              Limited Offer
            </span>
            <span className="truncate">
              🎉 Get <strong>20% OFF</strong> your first service booking! Use code <code className="bg-slate-950/20 px-1.5 py-0.5 rounded font-mono font-extrabold">LOCAL20</code> at checkout.
            </span>
          ...
          <button
            ...
            className="text-slate-950/70 hover:text-slate-950 p-1 rounded-md transition-colors shrink-0 ml-2"
      ...
      <header className="w-full border-b border-slate-800/80 bg-slate-900/90 backdrop-blur-md shadow-lg shadow-slate-950/20 text-white transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-[72px] flex items-center justify-between gap-4 lg:gap-6">
          ...
          <div className="hidden md:flex items-center gap-3 lg:gap-4 shrink-0">
            ...
            <div className="relative" ref={notifRef}>
              ...
              >
                ...
                {unreadCount > 0 && (
                  <span className="absolute top-2 right-2 flex h-4 w-4 items-center justify-center rounded-full bg-amber-500 text-[10px] font-extrabold text-slate-950 ring-2 ring-slate-900 animate-pulse">
                    {unreadCount}
                  </span>
                )}
              </Button>
              ...
              {showNotifications && (
                <div className="absolute right-0 mt-2 w-80 sm:w-96 bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl shadow-black/60 py-3 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                  ...
                  <div className="divide-y divide-slate-800/60 max-h-80 overflow-y-auto">
                    {notificationsList.map(item => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className={`p-3.5 flex items-start gap-3 hover:bg-slate-800/60 transition-colors cursor-pointer ${item.unread ? "bg-slate-800/30" : ""
                            }`}
                        >
                          ...
                          <div className="space-y-1 min-w-0 flex-1">
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span className="text-[10px] text-slate-500 font-medium shrink-0">{item.time}</span>
                            </div>
                            <p className="text-xs text-slate-400 leading-snug font-normal line-clamp-2">
                              {item.desc}
                            </p>
                          </div>
                        </div>
                  ...
                  <div className="pt-2 px-4 border-t border-slate-800 text-center">
                    <NavLink
                      to="/notifications"
                      onClick={() => setShowNotifications(false)}
                      className="text-xs text-slate-400 hover:text-amber-400 font-bold transition-colors block py-1"
                    >
          ...
          <div className="flex lg:hidden items-center gap-2">
            ...
            <Button
              ...
              size="icon"
              onClick={() => setShowNotifications(!showNotifications)}
              className="relative hover:bg-slate-800 text-slate-300 rounded-xl h-10 w-10 transition-colors"
            >
