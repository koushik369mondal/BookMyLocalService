    ...
    <DashboardLayout>
      ...
      <section className="bg-gradient-to-r from-slate-900 via-slate-700 to-slate-900 text-white py-12 relative overflow-hidden rounded-2xl mb-8">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          ...
          <p className="text-slate-350 text-xs mt-1.5 font-medium">Quickly rebook your preferred local experts and providers</p>
        </div>
      </section>
      ...
      <div className="space-y-6">
        <Card className="border border-slate-100 shadow-2xs rounded-2xl bg-white p-6">
          <CardHeader className="p-0 pb-4 border-b border-slate-50">
            <CardTitle className="text-base font-extrabold text-slate-900">Saved Professionals</CardTitle>
          ...
          <CardContent className="p-0 pt-6">
            {mockFavorites.length === 0 ? (
              <div className="text-center py-12">
                ...
                <p className="text-sm font-bold text-slate-400">No favorites saved yet</p>
                <p className="text-xs text-slate-400 mt-1">Bookmark providers to see them here</p>
              </div>
            ) : (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {mockFavorites.map((provider) => (
                  <div key={provider.id} className="border border-slate-200 p-5 rounded-2xl bg-white relative flex flex-col justify-between hover:border-slate-300 hover:shadow-sm transition-all duration-300">
                    <div className="flex items-start gap-4">
                      <Avatar className="w-14 h-14 border border-slate-100 shadow-xs">
                      ...
                      <div className="space-y-1">
                        ...
                        <span className="inline-block text-[10px] font-bold text-slate-900 bg-slate-900/5 px-2 py-0.5 rounded-lg">
                          {provider.category}
                        </span>
                    ...
                    <div className="border-t border-slate-50 pt-4 mt-5 flex items-center justify-between flex-wrap gap-2.5">
                      <Link to="/booking">
                        <Button size="xs" className="bg-slate-900 hover:bg-black text-white rounded-xl h-8.5 text-[10px] font-bold flex items-center gap-1">
